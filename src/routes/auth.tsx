...
function AuthPage() {
  ...
  if (user) { void navigate({ to: "/", replace: true }); return null; }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signin") {
        ...
        else if (result.alreadyExists) setMessage("This email already has an unverified account. Use Resend confirmation below to receive a new verification email.");
        else if (result.needsConfirmation) setMessage("Account created. Check your email to confirm your account.");
  ...
  async function resend() {
    ...
    try {
      ...
      setMessage(error ? error.message : "A new confirmation email has been requested. Check your inbox and spam folder.");
  ...
  return <main className="flex min-h-screen items-center justify-center bg-background px-4 py-10">
    <section className="surface-glass w-full max-w-md rounded-3xl p-6 shadow-lift">
      ...
      <button type="button" className="mt-4 w-full rounded-2xl border border-border px-4 py-2 text-sm text-primary hover:bg-muted disabled:opacity-50" disabled={resendBusy} onClick={resend}>{resendBusy ? "Sending…" : "Resend confirmation email"}</button>
      <button type="button" className="mt-5 w-full text-sm text-primary underline-offset-4 hover:underline" onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); setMessage(""); }}>{mode === "signin" ? "Need an account? Sign up" : "Already have an account? Sign in"}</button>
    </section>
  </main>;
}
