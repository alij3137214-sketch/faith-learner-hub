...
export const Route = createFileRoute("/auth")({
  head: () => ({ meta: [{ title: "Sign in — Nahj AI" }] }),
  component: AuthPage,
});
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-background px-4 py-10">
      <section className="surface-glass w-full max-w-md rounded-3xl p-6 shadow-lift">
        <Link to="/" className="text-sm text-primary">← Nahj AI</Link>
        <h1 className="mt-8 font-display text-3xl font-semibold">{mode === "signin" ? "Welcome back" : "Create your account"}</h1>
        <p className="mt-2 text-sm text-muted-foreground">Email and password are required to use the app and save progress.</p>
