    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <div className="mt-6">
          <Link
            to="/"
            className="press inline-flex items-center justify-center rounded-2xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground"
          >
            Go home
          </Link>
        </div>
      ...
      <div className="max-w-md text-center">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <button
            ...
            className="press rounded-2xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground"
          >
            Try again
          </button>
          <a href="/" className="rounded-2xl border border-input px-5 py-2.5 text-sm font-medium">
            Go home
          </a>
        </div>
      </div>
  ...
  head: () => ({
    meta: [
      ...
      { title: "Nahj AI — Learn from Authentic Sources" },
      {
        name: "description",
        content:
          "Nahj AI turns authentic Islamic scholarship into guided learning paths, quizzes and a source-cited knowledge assistant.",
      },
...
function Gates({ children }: { children: ReactNode }) {
  const { profile, user, isGuest, refreshProfile } = useAuth();
  ...
  return (
    <>
      {!splashDone && (
        <SplashScreen
          onDone={() => {
            sessionStorage.setItem("nahj.splash", "1");
            setSplashDone(true);
          }}
        />
      )}
      {splashDone && needsDisclaimer && <DisclaimerGate onAccept={() => void acceptDisclaimer()} />}
      {children}
    </>
  );
}
