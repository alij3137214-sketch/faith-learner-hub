...
export const Route = createFileRoute("/library")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Knowledge Library — Nahj AI" },
      { property: "og:description", content: "Browse books, speeches, letters and articles from indexed scholars." },
    ],
  }),
...
function LibraryPage() {
  ...
  const [scholarId, setScholarId] = useState<string | undefined>(undefined);
  ...
  return (
    <AppShell title="Library" subtitle="Authentic, indexed sources">
      ...
      <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
        <Chip active={!scholarId} onClick={() => setScholarId(undefined)}>
          All
        </Chip>
        {(scholars ?? []).map((s) => (
          <Chip key={s.id} active={scholarId === s.id} onClick={() => setScholarId(s.id)}>
            {s.name}
          </Chip>
        ))}
      </div>
