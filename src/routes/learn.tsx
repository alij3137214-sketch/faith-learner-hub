...
function LearnPage() {
  ...
  return <AppShell title="Learn" subtitle="Structured paths, one lesson at a time">
    ...
    {!isError && <div className="space-y-3">{isLoading && [0,1,2].map(i => <Skeleton key={i} className="h-24 rounded-3xl" />)}{!isLoading && (paths ?? []).length === 0 && <p className="rounded-3xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">No published learning paths are available.</p>}{(paths ?? []).map(p => <a key={p.id} href={`/learn/${p.slug}`} className="surface-glass card-lift block rounded-3xl p-4"><div className="flex items-center gap-3"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl gradient-emerald text-xl">📗</span><div className="min-w-0 flex-1"><p className="truncate font-display font-semibold">{p.title}</p><p className="line-clamp-1 text-xs text-muted-foreground">{p.description}</p><p className="mt-1 text-[11px] font-semibold uppercase tracking-wide text-primary">{p.path_items?.length ?? 0} lessons · {p.difficulty}</p></div><ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" /></div></a>)}</div>}
  </AppShell>;
}
