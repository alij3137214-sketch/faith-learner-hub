...
export const askKnowledgeBase = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<AskResult> => {
    ...
    // Vercel is configured with the VITE_* variables used by the browser.
    // The previous server function only checked SUPABASE_* and therefore failed
    // before retrieval with the misleading "Supabase URL is required" error.
    const url = process.env["SUPABASE_URL"] ?? process.env["VITE_SUPABASE_URL"];
    const key = process.env["SUPABASE_PUBLISHABLE_KEY"] ?? process.env["VITE_SUPABASE_PUBLISHABLE_KEY"];
    if (!url || !key) throw new Error("Supabase configuration is missing on the server.");
    ...
    if (!apiKey) {
      return { found: true, answer: "I found these relevant passages in the source library:\n\n" + top.map((c) => c.text).join("\n\n"), citations };
    }
