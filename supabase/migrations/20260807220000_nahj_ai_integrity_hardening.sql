...
-- Progression and grading become database-authoritative. Browser clients may request
-- actions, but they cannot choose XP/coin values or read answer keys.
...
-- The answer column must never be exposed to learner-facing API roles.
...
-- Do not allow browser clients to write authoritative progression tables directly.
...
-- Keep normal profile preferences editable while preventing client-side authority
-- over XP, coins, level, streak, activity dates, or suspension state.
...
BEGIN
  ...
  IF length(NEW.display_name) > 80 OR length(NEW.scholar_title) > 120 THEN
...
-- Server-only reward primitive. It is intentionally not executable by API roles.
...
BEGIN
  ...
  IF v_inserted = 0 THEN
    SELECT to_jsonb(p) INTO v_profile FROM public.profiles p WHERE p.user_id = p_user_id FOR UPDATE;
...
CREATE OR REPLACE FUNCTION public.grant_achievement_internal(
  p_user_id uuid,
  p_code text
)
RETURNS boolean
...
BEGIN
  ...
  WHERE user_id = p_user_id AND mission_id = v_mission.id AND period_key = v_period
  FOR UPDATE;
  ...
  IF v_completed THEN
    RETURN jsonb_build_object('updated', false, 'completed', true, 'progress', v_progress);
  END IF;

  v_progress := least(v_mission.target, v_progress + p_amount);
  v_completed := v_progress >= v_mission.target;

  IF v_existing THEN
    UPDATE public.user_missions
    SET progress = v_progress, completed = v_completed, updated_at = now()
    WHERE user_id = p_user_id AND mission_id = v_mission.id AND period_key = v_period;
  ELSE
    INSERT INTO public.user_missions(user_id, mission_id, progress, completed, period_key)
    VALUES (p_user_id, v_mission.id, v_progress, v_completed, v_period);
  END IF;
  ...
  IF v_completed THEN
    ...
    PERFORM public.apply_progression_reward(
      p_user_id,
      'mission:' || p_mission.code || ':' || v_period,
      'mission',
      v_xp,
      v_coins
    );
  ...
  RETURN jsonb_build_object('updated', true, 'completed', v_completed, 'progress', v_progress, 'xp', v_xp, 'coins', v_coins);
END;
$$;
...
BEGIN
  ...
  v_reward := public.apply_progression_reward(v_user, 'document:' || p_document_id, 'document', v_doc.xp_reward, greatest(1, round(v_doc.xp_reward / 4.0))::integer);
  PERFORM public.bump_mission_internal(v_user, 'daily_read', 1);
  PERFORM public.grant_achievement_internal(v_user, 'first_read');

  SELECT count(*) INTO v_count FROM public.user_progress WHERE user_id = v_user AND completed = true AND document_id IS NOT NULL;
...
BEGIN
  ...
  v_reward := public.apply_progression_reward(v_user, 'path_item:' || p_path_item_id, 'path_item', v_item.xp_reward, greatest(1, round(v_item.xp_reward / 4.0))::integer);
...
BEGIN
  ...
  SELECT correct_count, total_count, score_percent, passed, answers, created_at
  INTO v_attempt.correct_count, v_attempt.total_count, v_attempt.score_percent, v_attempt.passed, v_attempt.answers, v_attempt.created_at
  FROM public.quiz_attempts
  WHERE user_id = v_user AND quiz_id = p_quiz_id AND passed = true
  ORDER BY created_at DESC LIMIT 1;

  IF FOUND THEN
    RETURN jsonb_build_object('alreadyPassed', true, 'correct', v_attempt.correct_count,
      'total', v_attempt.total_count, 'scorePercent', v_attempt.score_percent,
  ...
  IF v_passed THEN
    v_reward := public.apply_progression_reward(v_user, 'quiz:' || p_quiz_id, 'quiz', v_quiz.xp_reward, greatest(1, round(v_quiz.xp_reward / 4.0))::integer);
    PERFORM public.bump_mission_internal(v_user, 'weekly_quiz', 1);
    PERFORM public.grant_achievement_internal(v_user, 'first_quiz');
  END IF;

  RETURN jsonb_build_object('alreadyPassed', false, 'correct', v_correct,
    'total', v_total, 'scorePercent', v_percent, 'passed', v_passed,
...
-- The browser can still read its own progression, but cannot mint it.
...
-- Make sure internal reward functions are not reachable through PostgREST.
